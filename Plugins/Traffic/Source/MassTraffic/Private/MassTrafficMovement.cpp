...
namespace UE::MassTraffic
{
	...
	)
	{
		...
		{
			const float ObstacleAvoidanceBrakingSpeedFactor = GetObstacleAvoidanceBrakingSpeedFactor(DistanceToNext, MinimumDistanceToNextVehicle, IdealDistanceToNextVehicle,
			                                                                                         NextVehicleAvoidanceBrakingPower);
		...
		if (TimeToCollidingObstacle < ObstacleAvoidanceBrakingTime)
		{
			...
			const float ObstacleAvoidanceBrakingSpeedFactor = GetObstacleAvoidanceBrakingSpeedFactor(DistanceToCollidingObstacle, MinimumDistanceToObstacle, ObstacleAvoidanceBrakingDistance,
			                                                                                         ObstacleAvoidanceBrakingPower);
			const float MaxAvoidanceSpeed = SpeedLimit * ObstacleAvoidanceBrakingSpeedFactor;
			TargetSpeed = FMath::Min(TargetSpeed, MaxAvoidanceSpeed);
		}
	...
	// OBB intersection by separating axis theorem
	// Find one axis which separates the two OBBs from each other and we have no collision	
	bool OBBIntersect(const FVector& PositionA, const FQuat& RotationA, const FVector& ExtentsA, const FVector& PositionB, const FQuat& RotationB, const FVector& ExtentsB)
	{
		static FVector SepAxis[15];

		const FVector ForwardA = RotationA.GetForwardVector();
		const FVector RightA = RotationA.GetRightVector();
		const FVector UpA = RotationA.GetUpVector();

		const FVector ForwardB = RotationB.GetForwardVector();
		const FVector RightB = RotationB.GetRightVector();
		const FVector UpB = RotationB.GetUpVector();
		...
		SepAxis[6] = FVector::CrossProduct(ForwardA, ForwardB).GetSafeNormal();
		SepAxis[7] = FVector::CrossProduct(ForwardA, RightB).GetSafeNormal();
		SepAxis[8] = FVector::CrossProduct(ForwardA, UpB).GetSafeNormal();

		SepAxis[9] = FVector::CrossProduct(RightA, ForwardB).GetSafeNormal();
		SepAxis[10] = FVector::CrossProduct(RightA, RightB).GetSafeNormal();
		SepAxis[11] = FVector::CrossProduct(RightA, UpB).GetSafeNormal();

		SepAxis[12] = FVector::CrossProduct(UpA, ForwardB).GetSafeNormal();
		SepAxis[13] = FVector::CrossProduct(UpA, RightB).GetSafeNormal();
		SepAxis[14] = FVector::CrossProduct(UpA, UpB).GetSafeNormal();

		const FVector Dist = PositionA - PositionB;
		const FVector HalfPosA = ForwardA * ExtentsA.X + RightA * ExtentsA.Y + UpA * ExtentsA.Z;
		const FVector HalfNegA = -ForwardA * ExtentsA.X + RightA * ExtentsA.Y + UpA * ExtentsA.Z;
		const FVector HalfPosB = ForwardB * ExtentsB.X + RightB * ExtentsB.Y + UpB * ExtentsB.Z;
		const FVector HalfNegB = -ForwardB * ExtentsB.X + RightB * ExtentsB.Y + UpB * ExtentsB.Z;

		for (int32 I = 0; I < 15; ++I)
		{
			if (SepAxis[I] == FVector::ZeroVector)
				continue;

			const float R = FMath::Abs(FVector::DotProduct(Dist, SepAxis[I]));
			const float R0 = FMath::Abs(FVector::DotProduct(HalfPosA, SepAxis[I])) + FMath::Abs(FVector::DotProduct(HalfPosB, SepAxis[I]));
			const float R1 = FMath::Abs(FVector::DotProduct(HalfNegA, SepAxis[I])) + FMath::Abs(FVector::DotProduct(HalfNegB, SepAxis[I]));
	...
	{
		const FVector AgentFinalPosition = AgentLocation + AgentVelocity * TimeToCollide;
		const FVector ObstacleFinalPosition = ObstacleLocation + ObstacleVelocity * TimeToCollide;

		const FVector AgentExtent = FVector(AgentHalfLength, AgentHalfWidth, AgentHalfWidth);
		const FVector ObstacleExtent = FVector(ObstacleParams.HalfLength, ObstacleParams.HalfWidth, ObstacleParams.HalfWidth);

		return OBBIntersect(AgentFinalPosition, AgentRotation, AgentExtent, ObstacleFinalPosition, ObstacleRotation, ObstacleExtent);
	}
	...
	bool WillCollide(
		...
		const float TimeToCollide)
	{
		return WillCollide(AgentLocation, AgentRotation, AgentVelocity, AgentParams.HalfLength, AgentParams.HalfWidth, ObstacleLocation, ObstacleRotation, ObstacleVelocity, ObstacleParams, TimeToCollide);
	}
		...
		FMassTrafficVehicleLaneChangeFragment* LaneChangeFragment, bool& bIsVehicleStuck)
	{
		...
		else
		{
			...
			const FMassEntityHandle NearestNextVehicle = FindNearestTailVehicleOnNextLanes(NewCurrentLane, TransformFragment.GetTransform().GetLocation(), EntityManager,
			                                                                               EMassTrafficFindNextLaneVehicleType::Tail);
		...
		{
			NewCurrentLane.ForEachVehicleOnLane(EntityManager, [VehicleEntity](const FMassEntityView& VehicleMassEntityView, struct FMassTrafficNextVehicleFragment& NextVehicleFragment,
			                                                                   struct FMassZoneGraphLaneLocationFragment& LaneLocationFragment)
			{
	...
	)
	{
		...
		// Safety checks for - Remove current vehicle from it's current lane.	
		{
			if (Entity_Current_Behind.IsSet() && Entity_Current_Ahead.IsSet())
			{
				...
				{
					UE_LOG(LogMassTraffic, Error, TEXT("Current lane %s - Valid current behind vehicle - Valid current ahead vehicle - But current vehicle is also current lane tail vehicle."),
					       *TrafficLaneData_Current.LaneHandle.ToString());
					bAllGood = false;
				}

				if (Entity_Current_Behind == Entity_Current_Ahead)
				{
					UE_LOG(LogMassTraffic, Error, TEXT("Current lane %s - Valid current behind vehicle - Valid current ahead vehicle - But both the same vehicle."),
					       *TrafficLaneData_Current.LaneHandle.ToString());
					bAllGood = false;
				}
				if (Entity_Current_Behind == Entity_Current)
				{
					UE_LOG(LogMassTraffic, Error, TEXT("Current lane %s - Valid current behind vehicle - Valid current ahead vehicle - But current vehicle is also current behind vehicle."),
					       *TrafficLaneData_Current.LaneHandle.ToString());
					bAllGood = false;
				}
				if (Entity_Current_Ahead == Entity_Current)
				{
					UE_LOG(LogMassTraffic, Error, TEXT("Current lane %s - Valid current behind vehicle - Valid current ahead vehicle - But current vehicle is also current ahead vehicle."),
					       *TrafficLaneData_Current.LaneHandle.ToString());
					bAllGood = false;
				}
			}
			else if (Entity_Current_Behind.IsSet() && !Entity_Current_Ahead.IsSet())
			{
				if (TrafficLaneData_Current.TailVehicle == Entity_Current)
				{
					UE_LOG(LogMassTraffic, Error, TEXT("Current lane %s - Valid current behind vehicle - No valid current ahead vehicle - But current vehicle is also current lane tail vehicle."),
					       *TrafficLaneData_Current.LaneHandle.ToString());
					bAllGood = false;
				}
				...
				if (Entity_Current_Behind == Entity_Current)
				{
					UE_LOG(LogMassTraffic, Error, TEXT("Current lane %s - Valid current behind vehicle - No valid current ahead vehicle - But current vehicle is also current behind vehicle."),
					       *TrafficLaneData_Current.LaneHandle.ToString());
					bAllGood = false;
				}
			}
			else if (!Entity_Current_Behind.IsSet() && Entity_Current_Ahead.IsSet())
			{
				if (TrafficLaneData_Current.TailVehicle != Entity_Current)
				{
					UE_LOG(LogMassTraffic, Error,
					       TEXT(
						       "Current lane %s - No valid current behind vehicle - Valid current ahead vehicle - But current vehicle is not current lane tail vehicle - Is current lane tail vehicle valid? %d."
					       ), *TrafficLaneData_Current.LaneHandle.ToString(), TrafficLaneData_Current.TailVehicle.IsSet());
					bAllGood = false;
				}

				if (Entity_Current_Ahead == Entity_Current)
				{
					UE_LOG(LogMassTraffic, Error, TEXT("Current lane %s - No valid current behind vehicle - Valid current ahead vehicle - But current vehicle is also current ahead vehicle."),
					       *TrafficLaneData_Current.LaneHandle.ToString());
					bAllGood = false;
				}
			}
			else if (!Entity_Current_Behind.IsSet() && !Entity_Current_Ahead.IsSet())
			{
				if (TrafficLaneData_Current.TailVehicle != Entity_Current)
				{
					UE_LOG(LogMassTraffic, Error,
					       TEXT(
						       "Current lane %s - No valid current behind vehicle - No valid current ahead vehicle - But current vehicle is not current lane tail vehicle - Is current lane tail vehicle valid? %d."
					       ), *TrafficLaneData_Current.LaneHandle.ToString(), TrafficLaneData_Current.TailVehicle.IsSet());
					bAllGood = false;
				}
			}
		...
		// @TODO If 1 lane ahead of us, next vehicle should be it's tail. (If >1 lanes ahead, do nothing for now.)
		{
			if (Entity_Chosen_Behind.IsSet() && Entity_Chosen_Ahead.IsSet())
			{
				...
				{
					UE_LOG(LogMassTraffic, Error, TEXT("Chosen lane %s - Valid chosen behind vehicle - Valid chosen ahead vehicle - But current vehicle is also chosen behind vehicle."),
					       *Lane_Chosen.LaneHandle.ToString());
					bAllGood = false;
				}
				if (Entity_Chosen_Ahead == Entity_Current)
				{
					UE_LOG(LogMassTraffic, Error, TEXT("Chosen lane %s - Valid chosen behind vehicle - Valid chosen ahead vehicle - But current vehicle is also chosen ahead vehicle."),
					       *Lane_Chosen.LaneHandle.ToString());
					bAllGood = false;
				}
			}
			else if (Entity_Chosen_Behind.IsSet() && !Entity_Chosen_Ahead.IsSet())
			{
				if (Entity_Chosen_Behind == Entity_Current)
				{
					UE_LOG(LogMassTraffic, Error, TEXT("Chosen lane %s - Valid chosen behind vehicle - No valid chosen ahead vehicle - But current vehicle is also chosen behind vehicle."),
					       *Lane_Chosen.LaneHandle.ToString());
					bAllGood = false;
				}
			}
			else if (!Entity_Chosen_Behind.IsSet() && Entity_Chosen_Ahead.IsSet())
			{
				if (Lane_Chosen.TailVehicle != Entity_Chosen_Ahead)
				{
					UE_LOG(LogMassTraffic, Error,
					       TEXT(
						       "Chosen lane %s - No valid chosen behind vehicle - Valid chosen ahead vehicle - But chosen ahead vehicle is not also chosen lane tail vehicle - Chosen lane tail vehicle valid? %d."
					       ), *Lane_Chosen.LaneHandle.ToString(), Lane_Chosen.TailVehicle.IsSet());
					bAllGood = false;
				}

				if (Entity_Chosen_Ahead == Entity_Current)
				{
					UE_LOG(LogMassTraffic, Error, TEXT("Chosen lane %s - No valid chosen behind vehicle - Valid chosen ahead vehicle - But current vehicle is also chosen ahead vehicle."),
					       *Lane_Chosen.LaneHandle.ToString());
					bAllGood = false;
				}
			}
			else if (!Entity_Chosen_Behind.IsSet() && !Entity_Chosen_Ahead.IsSet())
			{
				...
				{
					UE_LOG(LogMassTraffic, Error, TEXT("Chosen lane %s - No valid chosen behind vehicle - No valid chosen ahead vehicle - But chosen lane has a tail vehicle."),
					       *Lane_Chosen.LaneHandle.ToString());
					bAllGood = false;
				}
			}
		...
		if (!bAllGood)
		{
			UE_LOG(LogMassTraffic, Error, TEXT("Failed in pre-safety-check, teleport from lane %s to lane %s abborted. See previous warning(s)."), *TrafficLaneData_Current.LaneHandle.ToString(),
			       *Lane_Chosen.LaneHandle.ToString());
			return false;
		}
		...
		// of it's lane. It will get a new next vehicle once it begins to go through the intersection.
		{
			...
			Lane_Chosen.ForEachVehicleOnLane(EntityManager, [Entity_Current](const FMassEntityView& VehicleMassEntityView, struct FMassTrafficNextVehicleFragment& NextVehicleFragment,
			                                                                 struct FMassZoneGraphLaneLocationFragment& LaneLocationFragment)
			{
		...
		{
			const float SpaceTakenByVehicle_Current = GetSpaceTakenByVehicleOnLane(RadiusFragment_Current.Radius, RandomFractionFragment_Current.RandomFraction,
		...
		{
			const float DistanceToNewNext = FMath::Max(
				(DistanceAlongLane_Chosen - LaneLocationFragment_Chosen_Behind->DistanceAlongLane) - RadiusFragment_Chosen_Behind->Radius - RadiusFragment_Current.Radius, 0.0f);
		...
		{
			const float DistanceToNewNext = FMath::Max(
				(ZoneGraphLaneLocationFragment_Chosen_Ahead->DistanceAlongLane - DistanceAlongLane_Chosen) - AgentRadiusFragment_Chosen_Ahead->Radius - RadiusFragment_Current.Radius, 0.0f);
